...
from collections import deque #Un dèque est une structure de données de type conteneur et qui s’utilise à la fois comme une pile (stack) et une file d’attente (queue).
...
class Labyrinthe:
    ...
    def solution(self, depart_ligne, depart_colonne, arrivee_ligne, arrivee_colonne):
        ...
        """
        cellule_depart = self.grille[depart_ligne][depart_colonne]
        cellule_cible = self.grille[arrivee_ligne][arrivee_colonne]
        current_cellule = cellule_depart
        chemin = []
        visites = []
        coor_multi =[]
        x = 0
        ...
        #while current_cellule != cellule_cible:
        for i in range(20):
            if current_cellule == cellule_cible:
                print("wsh la solution a été trouvé")
            else:
                print("on est à", y, x)
                if current_cellule in visites:
                    print("Le noeud actuel a déja été visité")#la gauche ou d
                    print(nbr_deplacement)
                    nbr_deplacement =  (x+y)-nbr_deplacement
                    print(nbr_deplacement)
                    for i in range(nbr_deplacement):
                        chemin.pop()
                    current_cellule = self.grille[coor_multi[0]][coor_multi[1]]
                    chemin.append('e')
                    y = y + 1
                    
                else:
                    
                    visites.append(current_cellule)
                    if current_cellule.mur_bas == False and current_cellule.mur_droit == False:
                        coor_multi = [x,y]
                        
                    if current_cellule.mur_bas == False:
                        chemin.append('s')
                        x = x + 1
                        nbr_deplacement = nbr_deplacement + 1

                    elif current_cellule.mur_droit == False:
                        chemin.append('e')
                        y = y + 1
                        nbr_deplacement = nbr_deplacement + 1
            current_cellule = self.grille[x][y]
            print("Etat cul de sac :",cul_de_sac)
            
                 
        #                 self.grille[x][y].mur_bas = True
        ...
        #print(cellules_visites)

#         print(self.grille[0][0].mur_droit)
#         print(self.grille[0][0].mur_bas)
#         print(self.grille[0][1].mur_droit)
#         print(self.grille[0][1].mur_bas)
...
class GfxHMI(tk.Tk):
    def __init__(self, largeur, hauteur):
        ...
        self.trace_du_chemin = None  # conserve la variable tracée pour pouvoir effacer le chemin
